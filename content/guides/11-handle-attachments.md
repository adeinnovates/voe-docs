...
Readable attachments contribute **derived-grade** text: their own search hits, their own citations. Include it with `includeDerived=true` when breadth beats certainty (research, recall); exclude it when only asserted text grounds an answer (compliance, drafting). Either way the grade is visible in every hit.
