...
1. **Raw first.** The original bytes are durably stored before any parsing. If everything downstream burned down, the evidence survives.
2. **Mechanical extraction before models.** Parsing is mechanical where possible. When a message is too mangled to read mechanically, a bounded repair lane may use a model, and its output is checked against the original before it can land.
